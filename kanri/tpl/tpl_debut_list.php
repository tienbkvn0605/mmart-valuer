        ...
        $('.detail_item').on('click', function(){
            ...
            const request = $.ajax({
                ...
                success: function(response) {
					...
					var str_sanchi = response.m_sanchi.split("-")[0];
					var str_kakochi = response.m_sanchi.split("-")[1];
					// console.log(str_sanchi);
		...
		<form method="post" id="f_base" name="f_base" action="<?php echo $_SERVER['PHP_SELF']; ?>" target="_self">
			<table class="table table-hover table-bordered top" id="check_tbl">
				...
				<tr id="<?php echo($v["ai_serial"]); ?>">
					...
					<td><?php echo($v["valeur_coname"]); ?></td>
					<!-- 商品名 -->
					<td class="text-truncate" style="max-width: 300px;"><div data-toggle="modal" data-target="#detail_item" style="cursor: pointer;text-decoration: underline;color: #1e87f0;"data-serial="<?php echo($v["ai_serial"]) ?>" class="detail_item"><?php echo (($v["m_item"])); ?></div></td>
					<!-- カテゴリー -->
					<td><?php echo($v["c_cate1"]); ?></td>
					<!-- 単価/単価単位 -->
					<td><?php echo number_format($v['m_tanka']).'円／'.$v['m_tanka_tani'] ;?></td>
